...
void messageError(int type){
  ...
  dialog = gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "Erreur\n");
  if(type == 0){
    ...
    gtk_message_dialog_format_secondary_text (GTK_MESSAGE_DIALOG (dialog), "Impossible de se connecter à la base de données\nVeuillez réessayer.");
  }
