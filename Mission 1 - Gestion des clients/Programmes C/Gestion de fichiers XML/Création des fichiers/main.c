...
int main(int argc, char ** argv){
    ...
    Data          data;
    int           qrcode = 1; //QRCODE RECUPERE CORRESPONDANT A LIDENTIFIANT DE LUTILSATEUR (A MODIFIER)
    ...
    if(mysql_real_connect(mysql, "127.0.0.1", "root","", "worknshare", 0, NULL, 0)){
        ...
        while((row = mysql_fetch_row(result))){
            ...
            sprintf(data.idUser, "%s", row[0]); //RECUPERE IDENTFIANT DE L'UTILISATEUR
            sprintf(data.name, "%s", row[1]); //RECUPERE LE NOM DE L'UTILISATEUR
        ...
        while((row = mysql_fetch_row(result))){
            ...
            sprintf(data.state, "%s", row[7]); //RECUPERE LETAT DE LA RESERVATION (SI DISPONIBLE OU OCCUPE)
        }
                        ...
                        APRES LA CREATION DU FICHIER XML
                            ENVOI DU FICHIER XML VERS LE SERVEUR SI
                                L'envoi des fichiers XML vers le Service Informatique s'effectura tous les 23:00
                                                                                ...
                                                                                **/
        error = checkHourBeforeSend(instant, t); //VERIFIE L'HEURE, SI 23:00 ON ENVOIE LES FICHIERS SUR LE SERVEUR
        browseXMLFiles();
